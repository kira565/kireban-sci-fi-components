...
export default {
  ...
  theme: {
    extend: {
      ...
      boxShadow: {
        boardedShadowWarning:
          'inset 0 0 0 1px rgba(var(--warning-glow-rgb), 0.7) 0 0 0 1px rgba(var(--warning-glow-rgb), 0.7)',
        boardedShadowDanger:
          'inset 0 0 0 1px rgba(var(--danger-glow-rgb), 0.7) 0 0 0 1px rgba(var(--danger-glow-rgb), 0.7)',
        boxShadowDecal: 'inset 0 0 1px calc(var(--glow-size) / 2) var(--shade-3)',
        boxShadowTile:
          '0 0 var(--gutter-d-size) var(--glow-color), 0 0 calc(var(--gutter-d-size) / 2) var(--glow-color)'
      },
      textShadow: {
        warning:
          '-1px 1px 0 rgba(var(--warning-glow-rgb), .5), 1px -1px 0 rgba(var(--warning-glow-rgb), .5), -1px -1px 0 rgba(var(--warning-glow-rgb), .5), 1px 1px 0 rgba(var(--warning-glow-rgb), .5)',
        danger:
          '-1px 1px 0 rgba(var(--danger-glow-rgb), .5), 1px -1px 0 rgba(var(--danger-glow-rgb), .5), -1px -1px 0 rgba(var(--danger-glow-rgb), .5), 1px 1px 0 rgba(var(--danger-glow-rgb), .5)'
      }
    }
  },
  plugins: [
